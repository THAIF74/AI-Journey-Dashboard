...
def show_projects():
    ...
    cursor.execute(
        "SELECT name, status FROM projects ORDER BY id DESC LIMIT 1"
    )
    ...
    if saved_project:
        current_project = (
            ...
            + "\nStatus: " + saved_project[1]
        )
    else:
        current_project = "No saved projects yet."
...
def show_ideas():
    ...
    # Load latest saved idea
    cursor.execute(
        "SELECT idea FROM ideas ORDER BY id DESC LIMIT 1"
    )

    saved_idea = cursor.fetchone()

    if saved_idea:
        current_idea = "Latest Idea: " + saved_idea[0]
    else:
        current_idea = "No saved ideas yet."
